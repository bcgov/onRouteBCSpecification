 ...
 | Boosters           | Single                 |
  ...
  Scenario: axle unit maximum
   Given a single-axle booster is attached to the rear of a semi-trailer
     ...
     And the calculated axle group weight produces a lower overload amount
  ...
  Scenario: 7.17 maximum
   Given a single-axle booster is attached to the rear of a semi-trailer
     And the calculated 7.17 axle group weight produces a higher overload amount
     And the bridge formula result is not applicable
 ...
 | Boosters           | Single                 |
  ...
  Scenario: axle unit maximum
   Given a single-axle booster is attached to the rear of a semi-trailer
     ...
     And the calculated axle group weight produces a lower overload amount
     And the bridge formula result is not applicable
    Then the user sees the violation: "Axle Unit X exceeds permittable weight Y due to interaxle spacing"

  Scenario: axle unit below maximum
    Given a single-axle booster is attached to the rear of a semi-trailer
  ...
  Scenario: 7.17 maximum
    Given a single-axle booster is attached to the rear of a semi-trailer
      And the axle unit actual weight is 9,100 kg
      And the calculated axle group weight produces an overload amount
      And the bridge formula result is not applicable
     Then the overload is the the axle group actual weight - calculated legal allowable axle group (7.17 or 7.16g)
